  ...
  if (file) {
    ...
    while (getline(&line, &linecap, file) > 0) {
      ...
      if (match_patterns(flags, line, patterns, matches)) {
        matched_lines++;
        if (!flags['c'] && !flags['l']) {
          handler_header(flags, filename, file_count);
          handler_number(flags, line_num);
          if (flags['o'] && !flags['v']) {
    ...
    handler_header_cl(flags, filename, file_count);
    handler_count(flags, matched_lines);
    handler_list_files(flags, filename, matched_lines);
    free(line);
    fclose(file);
  } else if (!flags['s']) {
...
void print_err(int err_code) {
  if (err_code == NON_EXISTENT_FLAG) print_usage_error();
  if (err_code == MALLOC_ERROR) print_malloc_error();
}
